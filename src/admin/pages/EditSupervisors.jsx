...
import EditUserModal from "../../components/EditUserModal";
import ViewUserModal from "../../components/ViewUserModal";
import { FaEye, FaEdit, FaTrash, FaUsers, FaUserGraduate } from "react-icons/fa";
...
export default function EditSupervisors() {
  ...
  const fetchUsers = async (signal) => {
    try {
      ...
      const enrichedSupervisors = supervisorsArr.map((supervisor) => {
        const supName = String(supervisor.fullname || "").trim().toLowerCase();

        const projectCount = projectsArr.filter(
          (p) => String(p.supervisor || "").trim().toLowerCase() === supName,
        ).length;

        const researchCount = researchArr.filter(
          (r) => String(r.supervisor || "").trim().toLowerCase() === supName,
        ).length;
